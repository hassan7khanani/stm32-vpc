...
/* USER CODE BEGIN PV */
  RTC_TimeTypeDef sTime,rtc_system_time ={0},rtc_starttime={0} ,total_system_time ={0},total_cycle_time = {0},current_cycle_time={0},HOCL_cycle_time={0},NAOH_cycle_time={0},system_time={0}, start_time={0};
  RTC_DateTypeDef sDate ;
  RTC_AlarmTypeDef sAlarm ;

  //global variables for the user settings
  //const uint32_t drain_solenoid_cycletimer=86400; //in seconds
  const uint32_t drain_solenoid_cycletimer=15; //in seconds
  ...
  uint32_t total_seconds_current=0;
	uint32_t total_seconds_current_time=0;
...
uint32_t rtc_days_passed = 0;  // Global variable to track the number of days passed
  ...
  typedef struct {
  uint32_t counter;
  RTC_TimeTypeDef cycle_hours;
  bool fault_status;
  RTC_TimeTypeDef system_hours;
...
RTC_TimeTypeDef rtc_systemtime_conversion() {
    char rtc_time_buff[100];
    RTC_TimeTypeDef rtc_system_time;

    char test_buff1[100];
    snprintf(test_buff1, sizeof(test_buff1), "In rtc systemtime conversion function \r\n");
    ...
    snprintf(rtc_time_buff, sizeof(rtc_time_buff), "----RTC time is %d:%d:%d \r\n", sTime.Hours, sTime.Minutes, sTime.Seconds);
    HAL_UART_Transmit(&huart2, (uint8_t *)rtc_time_buff, strlen(rtc_time_buff), 100);
    memset(rtc_time_buff, 0, sizeof(rtc_time_buff));

    snprintf(rtc_time_buff, sizeof(rtc_time_buff), "----RTC date is %d/%d/%d  %d \r\n", sDate.Year, sDate.Month, sDate.Date, sDate.WeekDay);
    HAL_UART_Transmit(&huart2, (uint8_t *)rtc_time_buff, strlen(rtc_time_buff), 100);
    memset(rtc_time_buff, 0, sizeof(rtc_time_buff));

    // Calculate total seconds for current and start times
    uint32_t total_seconds_currentTime = (uint32_t)sTime.Hours * 3600 + (uint32_t)sTime.Minutes * 60 + (uint32_t)sTime.Seconds;
    uint32_t total_seconds_start = (uint32_t)rtc_starttime.Hours * 3600 + (uint32_t)rtc_starttime.Minutes * 60 + (uint32_t)rtc_starttime.Seconds;

    // Calculate elapsed time in seconds since the device was powered on
     elapsed_seconds = total_seconds_currentTime - total_seconds_start;

    // Handling the midnight rollover
    // Adjust for the initial 24-hour period, if needed
    if (elapsed_seconds < 0) {
        elapsed_seconds += 24 * 3600;
    }

    // Add the number of full days passed (in seconds) to the elapsed time
    // Assuming 'rtc_days_passed' contains the number of days passed since power on
    elapsed_seconds += rtc_days_passed * 24 * 3600;
    uint32_t total_hours = elapsed_seconds / 3600;
    uint32_t remaining_seconds = elapsed_seconds % 3600;
    rtc_system_time.Hours = total_hours % 24;  // To display hours in 24-hour format
    rtc_system_time.Minutes = remaining_seconds / 60;
    rtc_system_time.Seconds = remaining_seconds % 60;

    snprintf(rtc_time_buff, sizeof(rtc_time_buff), "---RTC systemtime is %02d:%02d:%02d, total elapsed seconds is %d \r\n", 
             rtc_system_time.Hours, rtc_system_time.Minutes, rtc_system_time.Seconds, elapsed_seconds);
    HAL_UART_Transmit(&huart2, (uint8_t *)rtc_time_buff, strlen(rtc_time_buff), 100);
    memset(rtc_time_buff, 0, sizeof(rtc_time_buff));

    return rtc_system_time;
}
  ...
  Measurements measure_current_voltage(void) {
      ...
      snprintf(current_value,sizeof(current_value), "Average current is %f \r\n ", main_cell.cell_current);
  ...
  bool current_below_threshold_for_duration()
  {
    ...
    char tx_buffer[100];
    RTC_TimeTypeDef under_current_standby_system_time;
    RTC_TimeTypeDef under_current_standby_cycle_time;
    ...
    if (start_time_m==0 && start_time_s==0)
      {
      start_time_m = system_time.Minutes;
      start_time_s = system_time.Seconds;
      ...
      elapsed_time = (system_time.Minutes * 60 + system_time.Seconds) - (start_time_m * 60 + start_time_s);
    ...
    if (system_time.Seconds - start_time_s >= time_threshold) {
  ...
  void current_high_threshold_for_duration()
  {
    ...
    static uint32_t elapsed_time=0;
    char tx_buffer[100];
    RTC_TimeTypeDef over_current_standby_system_time;
    RTC_TimeTypeDef over_current_standby_cycle_time;
  ...
  bool fill_salt_tube()
  {
      ...
      if (!salt_tube_bottom() && !salt_tube_top())
      {
          ...
          {
              start_time_s = system_time.Seconds;
              start_time_m = system_time.Minutes; // Set the start time when filling starts
              snprintf(tx_buffer,sizeof(tx_buffer), "----In fill salt tube function start time is :%d:%d \r\n", start_time_m, start_time_s);
          ...
          snprintf(tx_buffer,sizeof(tx_buffer),"----Salt tube filling started---- \r\n");
          //HAL_UART_Transmit(&huart2, (uint8_t *)tx_buffer, strlen(tx_buffer), 120);

          // Calculate elapsed time in seconds
          elapsed_time = (system_time.Minutes * 60 + system_time.Seconds) - (start_time_m * 60 + start_time_s);
          snprintf(tx_buffer,sizeof(tx_buffer), "elapsed time of salt tube filling is :%d \r\n", elapsed_time);
          ...
          if (salt_tube_top() && elapsed_time <= salttube_fill_time_limit)
          {
              ...
              snprintf(tx_buffer,sizeof(tx_buffer),"Salt tube filled in 90 seconds \r\n");
             // HAL_UART_Transmit(&huart2, (uint8_t *)tx_buffer, strlen(tx_buffer), 100);
              memset(tx_buffer, 0, sizeof(tx_buffer));
      ...
      if (salt_tube_bottom() && salt_tube_top())
      {
          ...
          snprintf(tx_buffer,sizeof(tx_buffer),"Salt tube filling stopped \r\n");
          HAL_UART_Transmit(&huart2, (uint8_t *)tx_buffer, strlen(tx_buffer), 120);
...
{
    char tx_buffer[200];
    snprintf(tx_buffer, sizeof(tx_buffer), "############## In drain solenoid function #################\r\n");
    HAL_UART_Transmit(&huart2, (uint8_t*)tx_buffer, strlen(tx_buffer), 100);
    memset(tx_buffer, 0, sizeof(tx_buffer));
    
    snprintf(tx_buffer, sizeof(tx_buffer), "system_seconds are");
    // Check if 24 hours (86400 seconds) have passed
    if ((system_time.Seconds - drain_solenoidTime) >= drain_solenoid_cycletimer) // 24 hours in seconds
    {
        snprintf(tx_buffer, sizeof(tx_buffer), "                      $$$$$$$$$$$$$$$$$$$  \r\n");
        HAL_UART_Transmit(&huart2, (uint8_t*)tx_buffer, strlen(tx_buffer), 100);
        if (drain_solenoidState == 0)
        {
            snprintf(tx_buffer, sizeof(tx_buffer), "---------drain solenoid on for 5 seconds-------\r\n");
            HAL_UART_Transmit(&huart2, (uint8_t*)tx_buffer, strlen(tx_buffer), 100);
            memset(tx_buffer, 0, sizeof(tx_buffer)); 
            ...
            drain_solenoidTargetTime = system_time.Seconds + 5; // 5 seconds in the future
        }
    }

    // Check if 5 seconds have passed
    if (drain_solenoidState == 1 && system_time.Seconds >= drain_solenoidTargetTime)
    {
        snprintf(tx_buffer, sizeof(tx_buffer), "---------drain solenoid off -------\r\n");
        HAL_UART_Transmit(&huart2, (uint8_t*)tx_buffer, strlen(tx_buffer), 100);
        memset(tx_buffer, 0, sizeof(tx_buffer));
        ...
        drain_solenoidTime = system_time.Seconds; // Reset the timer
    }
}
  ...
  bool isvalid_system_time(const RTC_TimeTypeDef *rtc_system_time) {
      // Check for valid hours, minutes, seconds
      if (rtc_system_time->Hours >= 0 && 
          rtc_system_time->Minutes >= 0 && rtc_system_time->Minutes< 60 &&
          rtc_system_time->Seconds >= 0 && rtc_system_time->Seconds < 60 ) 
      {
  ...
  bool isvalid_totalcycle_time(const RTC_TimeTypeDef *total_cycle_time) {
      // cycletime->cycletime_h < 24 &&
      // Check for valid hours, minutes, seconds
      if (total_cycle_time->Hours >= 0 && 
          total_cycle_time->Minutes >= 0 && total_cycle_time->Minutes < 60 &&
          total_cycle_time->Seconds >= 0 && total_cycle_time->Seconds < 60 )
          {
          return true; // Valid time
      } else {
          return false; // Invalid time
  ...
  RTC_TimeTypeDef read_systemtime_in_flashmemory()
  {
      const uint32_t system_time_address = 0x08020000;
      RTC_TimeTypeDef read_system_time;

      if (isvalid_system_time((RTC_TimeTypeDef*)system_time_address)) {
          read_system_time = *(RTC_TimeTypeDef*)system_time_address; // Read SystemTime from flash memory

          // Check for invalid time values
          if (read_system_time.Hours < 0 || read_system_time.Minutes < 0 || read_system_time.Seconds < 0 )
              
          {
              // Initialize to 00:00:00.000 if any value is invalid
              read_system_time.Hours = 0;
              read_system_time.Minutes = 0;
              read_system_time.Seconds = 0;
          }
      }
      ...
      snprintf(buffer, sizeof(buffer), "System Time read: %02d:%02d:%02d \r\n",
              read_system_time.Hours, read_system_time.Minutes,
              read_system_time.Seconds);
      HAL_UART_Transmit(&huart2, (uint8_t*)buffer, strlen(buffer), 100);
  ...
  {
     char test_buff[50];
      snprintf(test_buff, sizeof(test_buff), " ---In write system time function \r\n");
      ...
      RTC_TimeTypeDef system_time_fromflash;
      if (!read_time)
      {
      ...
      snprintf(read_system_time_fromflash_buffer, sizeof(read_system_time_fromflash_buffer), 
              "!!!!! System Time received from flash: %02d:%02d:%02d \r\n",
              system_time_fromflash.Hours, system_time_fromflash.Minutes,
              system_time_fromflash.Seconds);
      ...
      total_seconds_flash = system_time_fromflash.Hours * 3600 + system_time_fromflash.Minutes * 60 + system_time_fromflash.Seconds;
      ...
      RTC_TimeTypeDef current_time =rtc_systemtime_conversion();
      char system_time_buffer[300];
      snprintf(system_time_buffer, sizeof(system_time_buffer), " --- current system time: %02d:%02d:%02d \r\n",
      ...
      total_seconds_current = current_time.Hours * 3600 + current_time.Minutes * 60 + current_time.Seconds;
      ...
      total_system_time.Hours = 1;
      total_seconds = 3;
      total_system_time.Minutes = 2;
      total_system_time.Seconds = 5 ;
      total_system_time.Hours = total_seconds / 3600;
      total_seconds %= 3600;
      total_system_time.Minutes = total_seconds / 60;
      total_system_time.Seconds = total_seconds % 60;
      char total_system_time_buffer[200];
      snprintf(total_system_time_buffer, sizeof(total_system_time_buffer), 
              "!!!!! total system time calculated is : %02d:%02d:%02d \r\n",
              total_system_time.Hours, total_system_time.Minutes,
              total_system_time.Seconds);
     ...
     write_to_flash(system_hours_address, sytemtime_byte_array, size_of_system_structure, FLASH_TYPEPROGRAM_BYTE);
      ...
      char buffer[200];
      snprintf(buffer, sizeof(buffer), "System Time written on flash: %02d:%02d:%02d \r\n",
              total_system_time.Hours, total_system_time.Minutes,
              total_system_time.Seconds);
      HAL_UART_Transmit(&huart2, (uint8_t*)buffer, strlen(buffer), 100);
  }
  ...
  RTC_TimeTypeDef read_total_cycle_time_in_flashmemory()
  {
      
      const uint32_t total_cycle_time_address = 0x08010000;
      RTC_TimeTypeDef read_totalcycle_time; 
      
      if (isvalid_totalcycle_time((RTC_TimeTypeDef*)total_cycle_time_address)) {
          read_totalcycle_time = *(RTC_TimeTypeDef*)total_cycle_time_address; // Read SystemTime from flash memory

          // Check for invalid time values
          if (read_totalcycle_time.Hours < 0 || read_totalcycle_time.Minutes < 0 || read_totalcycle_time.Seconds < 0 )
              
          {
              // Initialize to 00:00:00.000 if any value is invalid
              read_totalcycle_time.Hours = 0;
              read_totalcycle_time.Minutes = 0;
              read_totalcycle_time.Seconds = 0;
    ...
    // Format and transmit system time
      snprintf(buffer, sizeof(buffer), "Totalcycle Time read: %02d:%02d:%02d \r\n",
              read_totalcycle_time.Hours, read_totalcycle_time.Minutes,
              read_totalcycle_time.Seconds);
      HAL_UART_Transmit(&huart2, (uint8_t*)buffer, strlen(buffer), 100);
  ...
  void write_total_cycle_time_in_flashmemory()
  {
      ...
      static RTC_TimeTypeDef start_time, end_time; 
      uint32_t size_of_totalcycle_structure = sizeof(RTC_TimeTypeDef);
      uint8_t totalcycletime_byte_array[size_of_totalcycle_structure];
      static bool read_time=false;
      static uint32_t total_seconds_flash=0;
      static uint32_t total_seconds=0;
      static uint32_t total_seconds_current_time=0;	  
      //read total cycle hours from flash memory
      RTC_TimeTypeDef totalcycle_time_fromflash;
      ...
      snprintf(read_cycle_time_fromflash_buffer, sizeof(read_cycle_time_fromflash_buffer), 
              ...
              totalcycle_time_fromflash.Hours, totalcycle_time_fromflash.Minutes,
              totalcycle_time_fromflash.Seconds);
      HAL_UART_Transmit(&huart2, (uint8_t*)read_cycle_time_fromflash_buffer, strlen(read_cycle_time_fromflash_buffer), 100);			
      
      // Convert system times to seconds
      total_seconds_flash = totalcycle_time_fromflash.Hours * 3600 + totalcycle_time_fromflash.Minutes * 60 + totalcycle_time_fromflash.Seconds;
      char total_seconds_flash_buffer[200];
      ...
      snprintf(current_cycle_time_buffer, sizeof(current_cycle_time_buffer), " --- current cycle time: %02d:%02d:%02d \r\n",
              current_cycle_time.Hours, current_cycle_time.Minutes,
              current_cycle_time.Seconds);
      ...
      total_seconds_current = current_cycle_time.Hours * 3600 + current_cycle_time.Minutes * 60 + current_cycle_time.Seconds;
      char total_seconds_current_buffer[200];
      ...
      // Convert back to hours, minutes, and seconds
      total_cycle_time.Hours= total_seconds / 3600;
      total_seconds %= 3600;
      total_cycle_time.Minutes = total_seconds / 60;
      total_cycle_time.Seconds = total_seconds % 60;
      ...
      snprintf(total_cycle_time_buffer, sizeof(total_cycle_time_buffer), 
              ...
              total_cycle_time.Hours, total_cycle_time.Minutes,
              total_system_time.Seconds);
  ...
  bool cell_current_greater_than_zero() {
      ...
      if (cell.cell_current > 0 && Idle) {
          ...
          else if (system_time.Seconds - startTime > threshold) {
 ...
 // char buffer[100];
  ...
  {
      MasterFault=true;
      write_state_to_flash("faults"); // Writing "faults" to flash
      ...
      if (cell_control_fault)
      {
        ...
        if (!prev_cell_control_status)
        {
          ...
          if (!Iscellfillsolenoid && system_time.Seconds - cellfill_solenoid_time >= 5) 
          {
          // Turn on the cell solenoid for 5 seconds
          cellfill_solenoid(true);
          Iscellfillsolenoid = true;
          cellfill_solenoid_time = system_time.Seconds; // Update the last change time
          }

          else if (Iscellfillsolenoid && system_time.Seconds - cellfill_solenoid_time >= 5) 
          {
...
else
{
  ...
  void idle_state()
  {
    ...
    static RTC_TimeTypeDef low_pressure_system_hours;
    static RTC_TimeTypeDef low_pressure_cycle_hours;
    ...
    static RTC_TimeTypeDef Tankfill_time_exceed_system_hours;
    static RTC_TimeTypeDef Tankfill_time_exceed_cycle_hours;
    ...
    RTC_TimeTypeDef system_time_fromflash_code;
    RTC_TimeTypeDef totalcycle_time_code;     
  ...
  if (!Iscellfillsolenoid && system_time.Seconds - cellfill_solenoid_time >= 30) {
  ...
  else if (Iscellfillsolenoid && system_time.Seconds - cellfill_solenoid_time >= 5) 
  {
    ...
    if (Tankfill_time && Tankfill_timeexceed && !Resume_signal && !Tankfill_time_trend_correctpasscode )
    {
      ...
      Tankfill_time_exceed_hours=system_time.Hours;
      uint32_t total_elapsed_minutes = system_time.Seconds / 60; // Convert ticks to total elapsed minutes
      ...
      HAL_UART_Transmit(&huart2, (uint8_t*)buff_elapsedtime, strlen(buff_elapsedtime), 100);
       ...
       //read system time from flash
        ...
        char buffer_sys[200];
        // Format and transmit system time
        snprintf(buffer_sys, sizeof(buffer_sys), "System Time read: %02d:%02d:%02d\r\n",
                system_time_fromflash_code.Hours, system_time_fromflash_code.Minutes,
                system_time_fromflash_code.Seconds);
      ...
      // Format and transmit system time
        snprintf(buffer_cyc, sizeof(buffer_cyc), "Totalcycle Time read: %02d:%02d:%02d \r\n",
                totalcycle_time_code.Hours, totalcycle_time_code.Minutes,
                totalcycle_time_code.Seconds);
      ...
      if (Tankfill_timeexceed_timestamps[0] - Tankfill_timeexceed_timestamps[3 - 1] <= 2880) {
        Tankfill_timetrend_code=system_time_fromflash_code.Hours-totalcycle_time_code.Hours;
  ...
  {
    write_state_to_flash("production"); // Writing "production" to flash
    Production=true;
    ...
    priming_start_time = current_time.Seconds;
    ...
    static RTC_TimeTypeDef low_current_trend_system_time;
    static RTC_TimeTypeDef low_current_trend_cycle_time={0},tankfill_timer={0};
    ...
    static uint32_t HOCL_cycle_time_elapsed_time_inminutes=0;
      ...
      if (Priming_sequence)
      {
      if (current_time.Seconds - priming_start_time <= Priming_sequence_timer)
      {
        ...
        salttube_start_time=current_time.Seconds;
      
      if (((current_time.Seconds - salttube_start_time) >= 2))
      {
  ...
  if (productionON_sequence)
  {
    ...
    current_cycle_time.Hours=system_time.Hours - start_time.Hours;
    current_cycle_time.Minutes=system_time.Minutes - start_time.Minutes;
    current_cycle_time.Seconds=system_time.Seconds - start_time.Seconds;
    ...
    tankfill_timer.Hours=system_time.Hours - start_time.Hours;
    tankfill_timer.Minutes=system_time.Minutes - start_time.Minutes;
    tankfill_timer.Seconds=system_time.Seconds - start_time.Seconds;
    elapsed_time=tankfill_timer.Seconds + tankfill_timer.Minutes * 60 + tankfill_timer.Hours * 3600; 
    ...
    if (!HOCL_tank_level())
    {
    HOCL_cycle_time.Hours=system_time.Hours - start_time.Hours;
    HOCL_cycle_time.Minutes=system_time.Minutes - start_time.Minutes;
    HOCL_cycle_time.Seconds=system_time.Seconds - start_time.Seconds;

    HOCL_cycle_time_elapsed_time_inminutes=(HOCL_cycle_time.Seconds/60) + HOCL_cycle_time.Minutes  + HOCL_cycle_time.Hours * 60;
    ...
    if (!NAOH_tank_level())
    {
    NAOH_cycle_time.Hours=system_time.Hours - start_time.Hours;
    NAOH_cycle_time.Minutes=system_time.Minutes - start_time.Minutes;
    NAOH_cycle_time.Seconds=system_time.Seconds - start_time.Seconds;
    NAOH_current_production_duration_m=HOCL_cycle_time.Minutes;

    //calculate the NAOH solution production calculation
    NAOH_cycle_time_elapsed_time_inminutes=(NAOH_cycle_time.Seconds/60) + NAOH_cycle_time.Minutes  + NAOH_cycle_time.Hours * 60;
  ...
  if (Stop_signal || (NAOH_tank_level() && HOCL_tank_level()) || monitor_pressure() || overcurrent || undercurrent || lowcurrent_trend )
  {
    ...
    flushing_start_time=system_time.Seconds;
    if(Flushing_sequence)
    {
    // In flushing condtion	 
    //turn off the main cell signal ON
    if(system_time.Seconds - flushing_start_time <= Flushing_sequence_timer)
    {
    HAL_GPIO_WritePin(main_cell_power_pin_port,main_cell_power_pin_no,GPIO_PIN_RESET);

    //reset tankfill timer
    tankfill_timer.Hours=0;
    tankfill_timer.Minutes=0;
    tankfill_timer.Seconds=0;
    elapsed_time=0;
...
void read_state_from_flash() {
    ...
    memcpy(prev_state, (char*)address, sizeof(prev_state));
    prev_state[sizeof(prev_state) - 1] = '\0'; // Ensure null termination

    char buffer[50];
    if (strcmp(prev_state, "production") == 0) {
        snprintf(buffer, sizeof(buffer), "State: Production \r\n");
        Production=true;
        //production_state();
    }
     else if (strcmp(prev_state, "idle") == 0) {
        snprintf(buffer, sizeof(buffer), "State: Idle \r\n");
        Idle=true;
       // idle_state();

    }
    else if (strcmp(prev_state, "faults") == 0) {
...
//initialize I2C devices
    ...
    HAL_RTC_GetDate(&hrtc, &sDate, RTC_FORMAT_BIN);
    rtc_starttime=sTime;
    snprintf(rtc_time_buff, sizeof(rtc_time_buff), "rtc_start time is %02d:%02d:%02d \r\n", rtc_starttime.Hours, rtc_starttime.Minutes, rtc_starttime.Seconds);
    HAL_UART_Transmit(&huart2, (uint8_t *)rtc_time_buff, strlen(rtc_time_buff), 1000);
    write_state_to_flash("idle");
  ...
  while (1)
  {
    ...
    snprintf(time_buff, sizeof(time_buff), "rtc_time in seconds :%d \r\n",elapsed_seconds);
    HAL_UART_Transmit(&huart2, (uint8_t *)time_buff, strlen(time_buff), 1000);
    memset(time_buff,0,sizeof(time_buff));

    system_time = rtc_systemtime_conversion();
    currentTime=system_time.Seconds;
    ...
    if ((currentTime - lastSaveTime) >= 60) //write after 1 minute
      ...
      if ((!Start_signal && !Production && !MasterFault && !Resume_signal && !Tankfill_time_trend_correctpasscode && !monitor_pressure()) || Idle || strcmp(prev_state, "idle") == 0)	
      {
      snprintf(time_buff,sizeof(time_buff),"---------In ideal condtion-------\r\n");
      ...
      if ((Start_signal || Resume_signal || Tankfill_time_trend_correctpasscode || monitor_pressure() || Autocycle || !HOCL_tank_level() || !NAOH_tank_level() ) && (!Idle_systemready_state && !Idle_tanksfull_state && !Idle_tankfill_timeexceeded_state && !Idle_lowpressure_state && !MasterFault) || (strcmp(prev_state, "production") == 0) )
      {
