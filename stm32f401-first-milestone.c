...
/* USER CODE BEGIN 0 */
  ...
  bool current_high_threshold_for_duration()
  {
...
// function to turn on the saltfill actuator
...
void read_state_from_flash() {
  ...
  char* monitor_faults()
  {
      ...
      bool under_current=current_below_threshold_for_duration();
      bool over_current=current_high_threshold_for_duration();
      ...
      static bool prev_under_current_status=false;
      static bool prev_over_current_status=false;
      ...
      static uint32_t fault_under_current_counter=0;
      ...
      if (under_current)
      {
          strcat(fault_name, "under_current ");
          fault_under_current_status=true;
          if(!prev_under_current_status)
          {
          fault_under_current_count++;
          if (fault_under_current_count>20){fault_under_current_count=0;}
          //update fault_t structure    
          fault_t fault_under_current;
          fault_under_current.counter=fault_under_current_counter;
          fault_under_current.fault_status=fault_under_current_status;
          fault_under_current.system_hours=system_time;
          fault_under_current.cycle_hours=current_cycle_time;
          fault_under_current.real_time_clock=sTime;
          update_fault(&my_faults,FAULT_TYPE_UNDER_CURRENT , &fault_under_current_count, fault_under_current);
          prev_under_current_status=true;
          
          }
          else
          {
            prev_under_current_status=false;
          }
      }
      ...
      {
          strcat(fault_name, "under_current ");
          fault_over_current_status=true;
          if(!prev_over_current_status)
          {
          fault_over_current_count++;
          if (fault_over_current_count>20){fault_over_current_count=0;}
          //update fault_t structure
          fault_t fault_over_current;
          fault_over_current.counter=fault_over_current_counter;
          fault_over_current.fault_status=fault_over_current_status;
          fault_over_current.system_hours=system_time;
          fault_over_current.cycle_hours=current_cycle_time;
          fault_over_current.real_time_clock=sTime;
          update_fault(&my_faults,FAULT_TYPE_OVER_CURRENT , &fault_over_current_count, fault_over_current);
          prev_over_current_status=true;

          }
          else
          {
            prev_over_current_status=false;
          }
      }
       //salt tube fault
      if (salt_tube_fault)
